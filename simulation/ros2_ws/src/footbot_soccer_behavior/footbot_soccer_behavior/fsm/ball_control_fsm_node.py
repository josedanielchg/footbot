...
import rclpy
from footbot_common.topics import CMD_VEL, SOCCER_BALL_STATE, SOCCER_FSM_STATE
...
class BallControlFsm(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('fsm_state_topic', SOCCER_FSM_STATE)
    ...
    def update_state(self, now):
        ...
        # Keep transitions explicit: each state decides only the next state,
        # while skill methods decide the low-level Twist for that state.
    ...
    def shutdown(self):
        try:
            if rclpy.ok():
                # Never leave a nonzero velocity command active after the FSM exits.
