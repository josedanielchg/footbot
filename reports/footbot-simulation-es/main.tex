...
\usepackage[spanish,es-tabla,es-noshorthands]{babel} % es-noshorthands: evita que ',' '>' choquen con TikZ
\usepackage{lmodern}
...
\usepackage{parskip} % línea en blanco entre párrafos (más fácil de leer)
...
\paragraph{¿Cómo mantiene el balón centrado? (paso a paso).}
La idea es sencilla: el robot \textbf{solo ve el balón en la imagen} de la cámara, no conoce su posición
real. Que el balón esté \emph{centrado} significa que aparece en el \textbf{centro horizontal} de la
imagen. El robot mide cuánto se desvía de ese centro y gira para corregirlo, igual que cuando caminamos
hacia algo que estamos mirando.

\textbf{Paso 1 — Medir qué tan descentrado está.} Si el balón aparece en la columna $u$ (en píxeles) de
una imagen de ancho $W$, el centro está en $W/2$. Esa desviación se traduce a un \emph{ángulo de rumbo}
$\theta$ (cuánto tendría que girar el robot para mirarlo de frente):
\begin{equation}
\theta = \arctan\!\left(\frac{u - W/2}{f}\right).
\end{equation}
Si el balón está justo en el centro, $\theta = 0$; si está a la derecha, $\theta > 0$; si está a la
izquierda, $\theta < 0$. (El factor $f$ solo traduce píxeles a ángulos según el lente de la cámara.)

\textbf{Paso 2 — Estimar a qué distancia está.} Cuanto más grande se ve el balón, más cerca está. A
partir de su radio aparente $r$ (px) y su radio real conocido $R$:
\begin{equation}
d = \frac{f\,R}{r}.
\end{equation}

\textbf{Paso 3 — Girar para centrarlo.} El robot gira con una velocidad angular $\omega$
\textbf{proporcional al error}: si el balón está muy descentrado, corrige rápido; cuando ya está casi
centrado, gira suave:
\begin{equation}
\omega = -\,K_p\,\theta .
\end{equation}
Aquí $K_p$ es solo ``cuánto'' corrige, y el signo negativo significa ``gira hacia el lado que reduce el
error''. Además, el comando se \textbf{suaviza} (se mezcla con el anterior) para que el giro no sea brusco
y se \textbf{limita} a una velocidad máxima de seguridad.

\textbf{Paso 4 — Decidir que ``ya lo controla''.} No basta con un instante: el robot da el balón por
\emph{controlado} solo si lleva un tiempo $T_{\mathrm{hold}}$ con el balón \textbf{centrado y cerca} a la
vez:
\begin{equation}
\lvert\theta\rvert \le \theta_{\mathrm{tol}}
\quad\text{y}\quad
d \le d_{\mathrm{ctrl}}
\quad\text{durante}\ \ge T_{\mathrm{hold}}.
\end{equation}
En la simulación se usan $K_p = 0{,}9$, $\theta_{\mathrm{tol}} = 0{,}18$~rad ($\approx 10^\circ$),
$d_{\mathrm{ctrl}} = 0{,}20$~m y $T_{\mathrm{hold}} = 0{,}35$~s.
...
la matriz de confusión del entrenamiento (Figs.~\ref{fig:confmat-norm} y~\ref{fig:confmat-counts}) muestra
el balón clasificado correctamente en el 100\,\% de los casos y la portería en el 90\,\% (con un 10\,\% de
porterías no detectadas, confundidas con el fondo). Es decir, \textbf{en las imágenes del \emph{dataset} la
percepción es fiable}. La limitación real no está en el \emph{dataset}, sino \textbf{en despliegue, cuando
el robot se acerca demasiado a la portería}: a corta distancia la portería deja de caber en el cuadro de la
cámara y el detector ya no la reconoce de forma continua, lo que degrada el remate
(Sección~\ref{sec:resultados-exp}).

\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{figures/metrics/yolo_confusion_matrix_normalized.png}
\caption{Matriz de confusión \textbf{normalizada} del entrenamiento de YOLO
(\texttt{ball}\,+\,\texttt{goal}) sobre el conjunto de validación: el balón se clasifica correctamente el
100\,\% de las veces y la portería el 90\,\%. En el \emph{dataset} la percepción es fiable.}
\label{fig:confmat-norm}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{figures/metrics/yolo_confusion_matrix.png}
\caption{Matriz de confusión en \textbf{conteos absolutos} (mismos datos de la
Fig.~\ref{fig:confmat-norm}). El problema de detección no aparece en el \emph{dataset}, sino sólo a muy
corta distancia durante la simulación (Sección~\ref{sec:resultados-exp}).}
\label{fig:confmat-counts}
\end{figure}
